(function () {
  ...
  var sortLbl = E('span', 'color:#bbb;');
  var sortSel = E('select', SELECT + 'padding:4px 7px;font-size:12px;');
  [['name-asc',''], ['name-desc',''], ['skill-asc','']].forEach(function (x) {
    var o = E('option'); o.value = x[0]; sortSel.appendChild(o);
  });
  sortSel.value = 'name-asc';

  var skillLbl = E('span', 'color:#bbb;');
  var skillSel = E('select', SELECT + 'padding:4px 7px;font-size:12px;max-width:260px;');
  var skillAllOpt = E('option'); skillAllOpt.value = ''; skillSel.appendChild(skillAllOpt);

  var shownCount = E('span', 'color:#888;');

  phead.appendChild(ptitle);
  phead.appendChild(sortLbl);
  phead.appendChild(sortSel);
  phead.appendChild(skillLbl);
  phead.appendChild(skillSel);
  phead.appendChild(shownCount);
  ...
  var busy = false;
  var devices = [];      /* [{name, id, skillId, skill}] */
  ...
  async function loadPhoenixMeta() {
    try {
      var r = await fetchText('/api/phoenix', { headers: { 'Accept': 'application/json' }, cache: 'no-store' });
      var j = parseJson(r.txt);
      if (j === undefined) return {};
      return buildPhoenixMeta(j);
    } catch (e) {
      return {};
    }
  }
  ...
  function visibleDevices() {
    var f = skillSel.value;
    var a = devices.filter(function (d) { return !f || (d.skillKey === f); }).slice();
    var mode = sortSel.value;
    function cmp(a, b) { return String(a || '').localeCompare(String(b || ''), undefined, { sensitivity: 'base' }); }
    a.sort(function (x, y) {
      if (mode === 'name-desc') return cmp(y.name, x.name);
      if (mode === 'skill-asc') return cmp(skillLabel(x), skillLabel(y)) || cmp(x.name, y.name);
      return cmp(x.name, y.name);
    });
    return a;
  }

  function selectable() { return visibleDevices().filter(function (d) { return d.id; }); }
  function selection() { return devices.filter(function (d) { return d.id && selected[d.id]; }); }

  function refreshSkillOptions() {
    var keep = skillSel.value;
    var found = {};
    devices.forEach(function (d) {
      if (d.skillKey) found[d.skillKey] = skillLabel(d);
    });
    while (skillSel.options.length > 1) skillSel.remove(1);
    Object.keys(found).sort(function (a, b) {
      return found[a].localeCompare(found[b], undefined, { sensitivity: 'base' });
    }).forEach(function (key) {
      var o = E('option', '', found[key]);
      o.value = key;
      skillSel.appendChild(o);
    });
    skillSel.value = found[keep] ? keep : '';
  }

  function updateSel() {
    var n = selection().length;
    var vis = selectable();
    var selectedVisible = vis.filter(function (d) { return selected[d.id]; }).length;
    selCount.textContent = t('selected', { n: n });
    shownCount.textContent = t('shown', { n: visibleDevices().length, total: devices.length });
    ...
    selAll.checked = vis.length > 0 && selectedVisible === vis.length;
    selAll.indeterminate = selectedVisible > 0 && selectedVisible < vis.length;
    selAll.disabled = vis.length === 0;
  }
  ...
  function applyLang() {
    ...
    skillAllOpt.textContent = t('allSkills');
    if (sortSel.options.length >= 3) {
      sortSel.options[0].textContent = t('sortNameAZ');
      sortSel.options[1].textContent = t('sortNameZA');
      sortSel.options[2].textContent = t('sortSkillAZ');
    }
    refreshSkillOptions();
    renderList();
    renderGeo();
